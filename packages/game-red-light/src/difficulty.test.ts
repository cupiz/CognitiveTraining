...
describe("getDifficultyConfig", () => {
  ...
  it("returns valid config for D5", () => {
    ...
    expect(config.initialStopSignalDelayMs).toBe(430);
  ...
  it("returns valid config for D10", () => {
    ...
    expect(config.initialStopSignalDelayMs).toBe(300);
  ...
  it("stop window is 1000ms per spec", () => {
    expect(STOP_WINDOW_MS).toBe(1000);
  });
});
